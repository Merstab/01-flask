...
class Minion {
  ...
  private readonly _wsMessagesRateLimit: (ws: any) => boolean = RateLimit(
  ...
  constructor(
    private readonly _nodeEndpoint: string,
    private readonly _markets: ZoMarketInfo[]
  ) {
    ...
    this._tid = setInterval(() => {
      logger.log(
        "debug",
        `Open WS client connections count: ${this._openConnectionsCount}`,
        meta
      );
    }, 60 * 1000);
  }
  ...
  public processMessages(messages: MessageEnvelope[]) {
    for (const message of messages) {
      ...
      if (logger.level === "debug") {
        const diff =
          new Date().valueOf() - new Date(message.timestamp).valueOf();
        logger.log(
          "debug",
          `Processing message, topic: ${topic}, receive delay: ${diff}ms`,
          meta
        );
      }
  ...
  private async _handleSubscriptionRequest(ws: WebSocket, buffer: ArrayBuffer) {
    try {
      ...
      if (validationResult.isValid === false) {
        logger.log(
          "debug",
          `Invalid subscription message received, error: ${validationResult.error}`,
          {
            message,
            ...meta,
          }
          ...
          if (request.op === "subscribe") {
            ...
            if (type === "recent_trades") {
              ...
              if (recentTrades !== undefined) {
                await this._send(
                  ws,
                  () => this._recentTradesSerialized[market]
                );
              } else {
                ...
                await this._send(ws, () =>
                  JSON.stringify(emptyRecentTradesMessage)
                );
              }
            }
      ...
      const errorMessage =
        typeof err === "string" ? err : `${err.message}, ${err.stack}`;
  ...
  private _validateRequestPayload(message: string) {
    ...
    if (OPS.includes(payload.op) === false) {
      return {
        isValid: false,
        error: `Invalid op: '${payload.op}'.${getDidYouMean(
          payload.op,
          OPS
        )} ${getAllowedValuesText(OPS)}`,
      } as const;
    }
...
if (minionNumber === 0) {
  setInterval(() => {
    const noDataPublishedForSeconds =
      (new Date().valueOf() - lastPublishTimestamp.valueOf()) / 1000;
