...
export function getAllowedValuesText(allowedValues: readonly string[]) {
  return `Allowed values: ${allowedValues
    .map((val) => `'${val}'`)
    .join(", ")}.`;
}
...
export const minionReadyChannel = new BroadcastChannel(
  "MinionReady"
) as BroadcastChannel;
export const zoProducerReadyChannel = new BroadcastChannel(
  "ZoProducerReady"
) as BroadcastChannel;
export const zoDataChannel = new BroadcastChannel("ZoData") as BroadcastChannel;
export const zoMarketsChannel = new BroadcastChannel(
  "ZoMarkets"
) as BroadcastChannel;
export const cleanupChannel = new BroadcastChannel(
  "Cleanup"
) as BroadcastChannel;
...
export async function getZoPerpMarkets(
  cluster: string,
  endpoint: string
): Promise<ZoMarketInfo[]> {
  const connection = new Connection(endpoint);
  const provider = createProvider(connection, undefined!);

  const zoCluster = cluster === "devnet" ? Cluster.Devnet : Cluster.Mainnet;
  const zoStateKey =
    cluster === "devnet" ? ZO_DEVNET_STATE_KEY : ZO_MAINNET_STATE_KEY;
  const zoDexKey =
    cluster === "devnet" ? ZO_DEX_DEVNET_PROGRAM_ID : ZO_DEX_MAINNET_PROGRAM_ID;
