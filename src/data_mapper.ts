...
export class DataMapper {
  ...
  private readonly _recentTrades: CircularBuffer<Trade> = new CircularBuffer(
    100
  );
  ...
  constructor(
    private readonly _options: {
      ...
      readonly market: ZoMarket; // ZoMarket
  ...
  public *map({
    accountsData,
    slot,
  }: AccountsNotificationPayload): IterableIterator<MessageEnvelope> {
    ...
    if (this._initialized && accountsData.eventQueue !== undefined) {
      ...
      for (const event of this._getNewlyAddedEvents(accountsData.eventQueue)) {
        ...
        if (event.eventFlags.fill === true && event.eventFlags.maker === true) {
          const makerFill: Fill = this._mapEventToDataMessage(
            event,
            timestamp,
            slot,
            fillsIds
          ...
          const currentOpenOrders =
            makerFill.side === "buy" ? newBidsOrders! : newAsksOrders!;
          ...
          if (hasMatchingOpenOrder === false) {
            const matchingOpenOrder = l3Diff.find(
              (l) => l.orderId === makerFill.orderId && l.type === "open"
            );
        ...
        const message = this._mapEventToDataMessage(
          event,
          timestamp,
          slot,
          fillsIds
    ...
    if (accountsData.asks !== undefined) {
      if (this._initialized) {
        const currentAsksMap = new Map(
          this._asksAccountOrders!.map(this._toMapConstructorStructure)
        );

        for (const ask of newAsksOrders!) {
          const matchingExistingOrder = currentAsksMap.get(ask.orderId);
          this._addChangedOrderItemsToL3Diff(
    ...
    if (accountsData.bids !== undefined) {
      if (this._initialized) {
        const currentBidsMap = new Map(
          this._bidsAccountOrders!.map(this._toMapConstructorStructure)
        );
    ...
    const snapshotHasChanged =
      ...
      (accountsData.asks !== undefined || accountsData.bids !== undefined);

    if (
      shouldInitialize ||
      snapshotHasChanged ||
      this._l3SnapshotPublishRequested
    ) {
      ...
      if (this._l3SnapshotPublishRequested) {
        ...
        this._localAsksOrdersMap = new Map(
          this._asksAccountOrders!.map(this._toMapConstructorStructure)
        );
        this._localBidsOrdersMap = new Map(
          this._bidsAccountOrders!.map(this._toMapConstructorStructure)
        );
    ...
    const asksDiff =
      accountsData.asks !== undefined
        ? this._getL2Diff(this._currentL2Snapshot.asks, newL2Snapshot.asks)
        : [];

    const bidsDiff =
      accountsData.bids !== undefined
        ? this._getL2Diff(this._currentL2Snapshot.bids, newL2Snapshot.bids)
        : [];
  ...
  private _addChangedOrderItemsToL3Diff(
    ...
    if (matchingExistingOrder === undefined) {
      const matchingFills = l3Diff.filter(
        (i) => i.type === "fill" && i.orderId === newOrder.orderId
      );
      ...
      if (matchingFills.length > 0) {
        for (const matchingFill of matchingFills) {
          ...
          size = (Number(size) + Number((matchingFill as any).size)).toFixed(
            this._options.sizeDecimalPlaces
          );
        }
      }

      const openMessage = this._mapToOrderMessage(
        newOrder,
        "open",
        size,
        timestamp,
      ...
      l3Diff.some(
        (i) => i.type === "fill" && i.orderId === newOrder.orderId && i.maker
      ) === false
    ) {
      // we have order change, can happen when  SelfTradeBehavior::DecrementTake?
      const changeMessage = this._mapToOrderMessage(
        newOrder,
        "change",
        newOrder.size,
        timestamp,
  ...
  private _validateL3DiffCorrectness(
    ...
    for (const item of l3Diff) {
      const ordersMap = (
        item.side === "buy"
          ? this._localBidsOrdersMap
          : this._localAsksOrdersMap
      )!;
      if (item.type === "open") {
      ...
      if (item.type === "fill") {
        ...
        if (matchingOrder !== undefined) {
          (matchingOrder as any).size = (
            Number((matchingOrder as any).size) - Number(item.size)
          ).toFixed(this._options.sizeDecimalPlaces);
  ...
  private _l2LevelChanged(
    currentLevel: PriceLevel | undefined,
    newLevel: PriceLevel | undefined
  ) {
  ...
  private _getPriceAsString(priceBN: BN) {
    ...
    if (priceString === undefined) {
      priceString = this._options.market
        .priceLotsToNumber(priceBN)
        .toFixed(this._options.priceDecimalPlaces);
  ...
  private _getSizeAsString(sizeBN: BN) {
    ...
    if (sizeString === undefined) {
      sizeString = this._options.market
        .baseSizeLotsToNumber(sizeBN)
        .toFixed(this._options.sizeDecimalPlaces);
  ...
  private _putInEnvelope(
    message: DataMessage | RecentTrades,
    publish: boolean
  ) {
  ...
  private _mapEventToDataMessage(
    ...
    const clientId = (event as any).clientOrderId
      ? (event as any).clientOrderId.toString()
      : undefined;
    ...
    if (event.eventFlags.fill) {
      const fillMessage: Fill = {
        ...
        clientId,
        side,
        price: this._getFillPrice(event).toFixed(
          this._options.priceDecimalPlaces
        ),
        size: this._getFillSize(event).toFixed(this._options.sizeDecimalPlaces),
        maker: event.eventFlags.maker,
        feeCost:
          this._options.market.quoteSplSizeToNumber(event.nativeFeeOrRebate) *
      ...
      let reason;
      const localOpenOrders =
        side === "buy" ? this._localBidsOrdersMap : this._localAsksOrdersMap;
      ...
      const doneMessage: Done = {
        ...
        sizeRemaining:
          reason === "canceled"
            ? this._getDoneSize(event).toFixed(this._options.sizeDecimalPlaces)
            : undefined,
        // version: this._version
      };
      return doneMessage;
    ...
    if (event.eventFlags.bid) {
      return this._options.market.baseSizeLotsToNumber(
        event.nativeQuantityReleased.div(
          event.orderId
            .ushrn(64)
            .mul((this._options.market as any)._decoded.quoteLotSize)
        )
      );
    } else {
      return divideBnToNumber(
  ...
  private *_getNewlyAddedEvents(eventQueueData: Buffer) {
    ...
    if (this._lastSeenSeqNum !== undefined) {
      const allocLen = Math.floor(
        (eventQueueData.length - HEADER.span) / NODE.span
      );

      const newEventsCount = Math.min(
        header.seqNum - this._lastSeenSeqNum,
        allocLen - 1
      );

      for (let i = newEventsCount; i > 0; --i) {
        const nodeIndex =
          (header.head + header.count + allocLen - i) % allocLen;
        const decodedItem = NODE.decode(
          eventQueueData,
          HEADER.span + nodeIndex * NODE.span
        ) as Event;
  ...
  private _mapToOrderItem = (
    { key, clientOrderId, feeTier, ownerSlot, control, quantity }: SlabItem,
    isBids: boolean
  ) => {
