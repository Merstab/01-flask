...
export class RPCClient {
  ...
  public async *streamAccountsNotification(
    ...
    wsEndpoint.protocol = this._options.nodeEndpoint.startsWith("https")
      ? "wss"
      : "ws";
    ...
    const accountsChangeNotifications = new AccountsChangeNotifications(
      market,
      options
    );
  ...
  async getAccountInfo(
    ...
    const { result } = await executeAndRetry(
      async () =>
        this._getAccountInfoRPCResponseRaw(publicKey.toBase58(), commitment),
      { maxRetries: 10 }
    );
  ...
  private async _getAccountInfoRPCResponseRaw(
    publicKey: string,
    commitment?: string
  ) {
...
class AccountsChangeNotifications {
  ...
  private _connectAndStreamData() {
    ...
    ws.onopen = async () => {
      try {
        ...
        const { accountsData, slot } = await executeAndRetry(
          async () => this._fetchAccountsSnapshot(),
          {
            maxRetries: 10,
          }
        );
        ...
        logger.log(
          "warn",
          `Failed to fetch accounts snapshot, ${err.message}`,
      ...
      logger.log("info", "Established new RPC WebSocket connection...", {
        market: this._options.marketName,
      });
    };
      ...
      if (message.method === "accountNotification") {
        ...
        if (matchingSubMeta !== undefined) {
          const accountData = Buffer.from(
            subMessage.params.result.value.data[0],
            "base64"
          );
    ...
    ws.onerror = (event) => {
      logger.log(
        "info",
        `Received RPC WebSocket error, error message: ${event.message}`,
        {
          market: this._options.marketName,
        }
      );
    ...
    ws.onclose = (event) => {
      logger.log(
        "info",
        `Received RPC WebSocket close, reason: ${event.reason}, code: ${event.code}`,
        {
          market: this._options.marketName,
        }
      );
  ...
  private async _fetchAccountsSnapshot() {
    ...
    try {
      ...
      for (let i = 0; i < data.result.value.length; i++) {
        ...
        accountsData[accountName] = Buffer.from(
          data.result.value[i]!.data[0],
          "base64"
        );
      }
  ...
  private async _restartConnection() {
    const delayMs =
      this._retriesCount > 0
        ? Math.min(Math.pow(2, this._retriesCount) * 1000, 32 * 1000)
        : 0;

    logger.log("info", "Restarting RPC WebSocket connection...", {
      market: this._options.marketName,
      delayMs,
    });
  ...
  private _monitorConnectionIfStale(ws: WebSocket) {
    ...
    this._staleConnectionTID = setInterval(() => {
      if (this._receivedMessagesCount === 0) {
        logger.log(
          "info",
          `Did not received any messages within 120s timeout, terminating connection...`,
          {
            market: this._options.marketName,
          }
        );
  ...
  private _publish = () => {
    ...
    if (this._slotStartTimestamp !== undefined) {
      ...
      if (slotTimespan > 400) {
        logger.log(
          "debug",
          `Slow accounts notification, slot ${this._currentSlot}, ${slotTimespan}ms`,
          {
            market: this._options.marketName,
          }
        );
  ...
  private _update(
    accountName: "bids" | "asks" | "eventQueue",
    accountData: Buffer,
    slot: number
  ) {
    if (logger.level === "debug") {
      logger.log(
        "debug",
        `Received ${accountName} account update, current state ${this._state}`,
        {
          market: this._options.marketName, //symbol
          slot,
        }
      );
    }
    ...
    if (this._state === "PRISTINE") {
      if (this._currentSlot === slot) {
        ...
        logger.log("warn", "Ignoring WS account notification", {
          market: this._options.marketName,
        });
        return;
      } else {
    ...
    if (this._state === "PENDING") {
      ...
      if (slot === this._currentSlot) {
        if (this._accountsData[accountName] !== undefined) {
          logger.log(
            "warn",
            `Received second update for ${accountName} account for the same slot`,
            {
              slot,
              market: this._options.marketName,
              accountName,
