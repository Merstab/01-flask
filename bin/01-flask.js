...
const argv = yargs
  ...
  .option("boot-delay", {
    type: "string",
    describe:
      "Staggered boot delay in milliseconds so public RPC nodes do not rate limit 01-flask",
    default: 500,
  })
...
async function start() {
  ...
  const bootTimeSeconds = Math.ceil(
    (new Date().valueOf() - startTimestamp) / 1000
  );

  if (isDocker()) {
    logger.log(
      "info",
      `01-flask v${pkg.version} is running inside Docker container.`,
      { bootTimeSeconds }
    );
  } else {
    logger.log(
      "info",
      `01-flask server v${pkg.version} is running on port ${port}.`,
      { bootTimeSeconds }
    );
  }
  ...
  logger.log(
    "info",
    `See https://github.com/Merstab/01-flask for more information.`
  );
}
